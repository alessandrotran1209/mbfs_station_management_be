...
class Operation():
    ...
    def complete_operation(self, operation):
        try:
            ...
            query = {"station_code": station_code, "date": operation_date,
                     "work_code": word_code}
    ...
    def search_operation(self, station_code='', start_date='', end_date='', page=1):
        mongo_conn = MongoConn()
        client = mongo_conn.conn()
        end_date = datetime.today()+ timedelta(days=1) if end_date == '' else datetime.strptime(end_date, '%d/%m/%Y') + timedelta(days=1)
        start_date = datetime(1, 1, 1, 0, 0) if start_date == '' else datetime.strptime(start_date, '%d/%m/%Y')

        operation_collection = client['operation']
        total = operation_collection.count_documents(
            {"station_code": {'$regex': station_code}, "date": {'$lt': end_date, '$gte': start_date}})
        records = operation_collection.find(
            {"station_code": {'$regex': station_code}, "date": {'$gte': start_date, '$lt': end_date}},
    ...
    def insert_operation(self, operation):
        try:
            ...
            operation_data = {"operator": operator, "station_code": station_code, "date": date, "work_code": work_code,
                              "status": status}
