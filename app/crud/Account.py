...
class Account:
    ...
    def get_statistics(self, operator, fullname, role, group):
        ...
        if role == 'operator':
            total_operation = operation_collection.count_documents(
                {"operator": operator, "group": {"$regex": group}})
            total_completed_operation = operation_collection.count_documents(
                {"operator": operator, 'status': "1", "group": {"$regex": group}})
            total_uncompleted_operation = operation_collection.count_documents(
                {"operator": operator, 'status': "0", "group": {"$regex": group}})
            total_station = station_collection.count_documents(
                {"operator": fullname, "group": {"$regex": group}})
    ...
    def get_role(self, username):
        ...
        records = group_leader_collection.find_one(
    ...
    def get_zone_statistics(self, user, zone):
        mongo_conn = MongoConn()
        client = mongo_conn.conn()
        operation_collection = client['operation']
